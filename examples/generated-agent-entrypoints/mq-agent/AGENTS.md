...
Generated from mqobsidian agent-entrypoint templates for mq-agent.
...
4. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory/learn/agent/mq-agent.md` if it exists.
5. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/systems/mq-agent/hot.md` if it exists.
6. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/systems/mq-agent/index.md` if it exists.
7. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory/learn/repos/mq-agent.md` if it exists.
...
uv --directory ${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory run python commands/emit_observation.py \
  --producer codex --repository mq-agent --workflow task-work --category pattern --confidence 0.70 \
  ...
  --evidence-excerpt "Concrete evidence summary" --tag mq-agent
```
