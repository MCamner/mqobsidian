...
Generated from mqobsidian agent-entrypoint templates for mq-ums.
...
Regenerate with:
  MQ_OBSIDIAN_DIR=<path-to-mqobsidian> \
    python3 "$MQ_OBSIDIAN_DIR"/scripts/generate-agents-md.py --repo mq-ums --out AGENTS.md
-->
...
4. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory/learn/agent/mq-ums.md` if it exists.
5. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/systems/mq-ums/hot.md` if it exists.
6. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/systems/mq-ums/index.md` if it exists.
7. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory/learn/repos/mq-ums.md` if it exists.
...
uv --directory ${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory run python commands/emit_observation.py \
  --producer codex --repository mq-ums --workflow task-work --category pattern --confidence 0.70 \
  ...
  --evidence-excerpt "Concrete evidence summary" --tag mq-ums
```
