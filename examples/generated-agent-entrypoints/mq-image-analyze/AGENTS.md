...
Generated from mqobsidian agent-entrypoint templates for mq-image-analyze.
...
Regenerate with:
  MQ_OBSIDIAN_DIR=<path-to-mqobsidian> \
    python3 "$MQ_OBSIDIAN_DIR"/scripts/generate-agents-md.py --repo mq-image-analyze --out AGENTS.md
-->
...
4. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory/learn/agent/mq-image-analyze.md` if it exists.
5. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/systems/mq-image-analyze/hot.md` if it exists.
6. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/systems/mq-image-analyze/index.md` if it exists.
7. Read `${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory/learn/repos/mq-image-analyze.md` if it exists.
...
uv --directory ${MQ_OBSIDIAN_DIR:-$HOME/mqobsidian}/memory run python commands/emit_observation.py \
  --producer codex --repository mq-image-analyze --workflow task-work --category pattern --confidence 0.70 \
  ...
  --evidence-excerpt "Concrete evidence summary" --tag mq-image-analyze
```
